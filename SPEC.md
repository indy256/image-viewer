...
| Right-click | Open a context menu in either window mode with fullscreen, previous/next image, copy image, sharpening (Off/Mild/Strong), gamma (increase/decrease/reset), and exit. Show current fullscreen and sharpening selections and gamma value; disable unavailable commands. Esc dismisses the menu without closing the viewer. |
...
Usage, invalid-startup-path, and empty-folder views use `Image Viewer version` as the title.
...
| macOS ARM64 | `iv-macos-arm64.dmg`, containing `iv.app` with its Qt frameworks and plugins. |
